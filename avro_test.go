...
func TestAvroToIceberg(t *testing.T) {
	...
	icebergSchema, err := iceberg.AvroToIceberg(avscManifestFile)
	assert.NoError(t, err)
	assert.Equal(t, expected.ID, icebergSchema.ID)
	assert.Equal(t, expected.Fields(), icebergSchema.Fields())
}
